...
export class NosotrosComponent implements OnInit {
  ...
  // ── Datos (mock hasta que exista el backend) ──────────────────
  mision = 'Acompañamos con confianza y compromiso el camino de cada proyecto.';
  vision = 'Construimos una relación fuerte y sólida con nuestros clientes.';

  valores: Valor[] = [
    {
      nombre: 'Calidad constructiva',
    ...
    {
      nombre: 'Eficiencia en diseño y gestión',
  ...
  ngOnInit(): void {
    setTimeout(() => {
      ...
      setTimeout(() => (this.mvVisible = true), 150);
      setTimeout(() => (this.valoresVisible = true), 300);
    }, 80);

    // Cuando exista el backend, cargar desde API
    this.http.get<any>('/api/nosotros').pipe(catchError(() => of(null))).subscribe(data => {
      if (data) {
        if (data.mision)  this.mision  = data.mision;
        if (data.vision)  this.vision  = data.vision;
        if (data.valores) this.valores = data.valores;
      }
    });
  }
}
